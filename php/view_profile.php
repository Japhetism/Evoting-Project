...
$sqm = $connection1->prepare("SELECT election_name, election_start_date,election_time_from FROM election WHERE election_id='$election_id_here'");
$sqm->execute();
$results = $sqm->setFetchMode(PDO::FETCH_ASSOC);
$results = $sqm->fetchAll();
$election_start = $results[0]["election_start_date"];
$election_name = $results[0]["election_name"];
$election_start_time= $results[0]["election_time_from"];

$edit_nick_nameErr=$edit_nick_name = $update_election_message="";

if ($_SERVER["REQUEST_METHOD"] == "POST") {

    if (strtotime(date("Y-m-d")) < strtotime($election_start) | ((strtotime(date("Y-m-d")) == (strtotime($election_start)))&&(strtotime(date("H:i:s"))>(strtotime($election_start_time)-(60*60))))) {
        ...
        $update_election_message="You cannot edit your profile for this election less than an hour to the election";
    }
}
...
$hasvoted=hasvoted(user_id($myemail), $election_id_here);
?>
